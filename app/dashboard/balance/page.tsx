...
import { ArrowDownLeft, ArrowUpRight, TrendingUp, TrendingDown, CreditCard, Building2 } from "lucide-react"
...
function getTxIcon(tx: any, userId: string) {
  if (tx.type === "DEPOSIT")    return { Icon: ArrowDownLeft, bg: "bg-[#E1F5EE]", color: "text-[#085041]" }
  if (tx.type === "WITHDRAWAL") return { Icon: Building2,     bg: "bg-[#FAEEDA]", color: "text-[#633806]" }
  if (tx.type === "PAYMENT")    return { Icon: CreditCard,    bg: String(tx.senderId) === userId ? "bg-[#FAEEDA]" : "bg-[#E1F5EE]", color: String(tx.senderId) === userId ? "text-[#633806]" : "text-[#085041]" }
  return String(tx.receiverId) === userId
    ...
    <div className="max-w-[900px]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-[#0a3d2e] rounded-[16px] p-5">
          <p className="text-[11px] font-semibold text-[#6fa890] tracking-widest mb-2">TOTAL BALANCE</p>
          <p className="text-[28px] font-bold text-white" style={{ fontFamily: "'Fraunces', serif" }}>
        ...
        <div className="bg-white border border-[#dde8e3] rounded-[16px] p-5">
          <div className="flex items-center justify-between mb-2">
            <p className="text-[11px] font-semibold text-[#8a9e96] tracking-widest">TOTAL INCOME</p>
            <div className="w-7 h-7 bg-[#E1F5EE] rounded-full flex items-center justify-center">
              <TrendingUp size={13} className="text-[#085041]" />
            </div>
          </div>
          <p className="text-[22px] font-bold text-[#0a3d2e]" style={{ fontFamily: "'Fraunces', serif" }}>
        ...
        <div className="bg-white border border-[#dde8e3] rounded-[16px] p-5">
          <div className="flex items-center justify-between mb-2">
            <p className="text-[11px] font-semibold text-[#8a9e96] tracking-widest">TOTAL SPENT</p>
      ...
      <div className="bg-white border border-[#dde8e3] rounded-[16px] overflow-hidden">
        ...
        {!data?.recentTransactions?.length ? (
          <p className="text-[13px] text-[#8a9e96] text-center py-10">No transactions yet</p>
        ) : (
          data.recentTransactions.map((tx: any) => {
            const { label, isCredit } = getTxLabel(tx, userId)
            const { Icon, bg, color } = getTxIcon(tx, userId)
            return (
              <div key={tx._id} className="flex items-center justify-between px-5 py-3.5 border-b border-[#f0f5f2] last:border-0 hover:bg-[#f6faf8] transition-colors">
